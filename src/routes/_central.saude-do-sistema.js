...
import { Activity, RefreshCcw, ShieldCheck, Zap, Database, Cloud, Cpu, Server, Loader2 } from "lucide-react";
import { KirvraAppShell } from "@/components/kirvra/app-shell";
import { MetricCard, PageHeader, Panel, StatusBadge, LoadingState } from "@/components/kirvra/primitives";
...
function HealthStatusBadge({ status }) {
    ...
    return (<StatusBadge tone={tones[status]}>
      {labels[status]}
    </StatusBadge>);
}
...
function SystemHealthPage() {
    ...
    if (loading) {
        return (<KirvraAppShell title="Saúde do sistema">
        <LoadingState label="Verificando infraestrutura..."/>
      </KirvraAppShell>);
    }
    return (<KirvraAppShell title="Saúde do sistema">
      <div className="space-y-6">
        <PageHeader title="Saúde do sistema" description="Monitoramento em tempo real da infraestrutura e serviços críticos da Central KIRVRA." actions={<div className="flex gap-2">
              <Button variant="outline" size="sm" className="gap-2" onClick={() => loadData(true)} disabled={refreshing || diagnosticating}>
                {refreshing ? <Loader2 className="h-3.5 w-3.5 animate-spin"/> : <RefreshCcw className="h-3.5 w-3.5"/>}
                Atualizar
              </Button>
              <Button size="sm" className="gap-2 bg-primary text-primary-foreground hover:bg-primary/90" onClick={handleDiagnostic} disabled={refreshing || diagnosticating}>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <MetricCard label="Disponibilidade" value={data?.availability || "—"} sublabel="Últimos 30 dias" tone={data?.availability !== "—" ? "success" : "neutral"}/>
          <MetricCard label="Latência média" value={data?.averageLatencyMs ? `${data.averageLatencyMs} ms` : "—"} sublabel="Medição atual" tone={data?.averageLatencyMs ? (data.averageLatencyMs < 200 ? "success" : "warning") : "neutral"}/>
          <MetricCard label="Fila de IA" value={data?.aiQueueSize !== undefined && data?.aiQueueSize !== null ? String(data.aiQueueSize) : "—"} sublabel="Processamentos pendentes" tone={data?.aiQueueSize === 0 ? "success" : "neutral"}/>
          <MetricCard label="Incidentes" value={String(data?.incidentCount || 0)} sublabel="Nas últimas 24h" tone={data?.incidentCount === 0 ? "success" : "critical"}/>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-border">
                    {data?.services?.map((service) => (<tr key={service.id} className="group hover:bg-muted/30">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            <div className={cn("flex h-8 w-8 items-center justify-center rounded-lg border", service.status === "online" ? "border-success/20 bg-success/5 text-success" :
          ...
          <div>
            <Panel title="Eventos Recentes" description="Log de alterações na infraestrutura.">
              {data?.recentEvents && data.recentEvents.length > 0 ? (<div className="space-y-4">
                  {data.recentEvents.map((event) => (<div key={event.id} className="relative pl-4 before:absolute before:left-0 before:top-2 before:h-full before:w-[1px] before:bg-border last:before:h-2">
                ...
                </div>) : (<div className="flex flex-col items-center justify-center py-10 text-center opacity-50">
