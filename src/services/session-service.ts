 ...
 */
import type { Alert, Driver, ProtectionSession, Vehicle } from "@/integrations/vyra/types";
import { isVyraConfigured } from "@/integrations/vyra/client";
import { alerts, findDriver, findSession, findVehicle } from "@/mocks/kirvra-central";
...
export async function addSessionNote(_sessionId: string, note: string): Promise<ServiceResult> {
  assertDemoData();
...
export async function escalateSession(_sessionId: string, reason: string): Promise<ServiceResult> {
  assertDemoData();
